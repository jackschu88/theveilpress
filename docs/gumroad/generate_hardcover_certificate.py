"""
Generate Hardcover Book certificate + receipt PDF for Gumroad delivery.
Run: python generate_hardcover_certificate.py
"""
...
def build():
    ...
    c.setTitle("Hardcover Book — Certificate of Purchase & Receipt")
    c.setAuthor("The Veil Press")
    c.setSubject("Hardcover edition — certificate and receipt")
    ...
    y -= 28
    draw_centered(c, "HARDCOVER EDITION", y, "Georgia-Bold", 14, GOLD)
    y -= 16
    draw_centered(c, "Casebound Volume", y, "Georgia-Italic", 12, INK_SOFT)
    ...
    body = (
        ...
        "edition of The Veil of the Square Mile — Volume I from The Veil Press."
    )
    y = wrap_centered(
    ...
    y -= 14
    includes = [
        "One hardcover copy of The Veil of the Square Mile",
        "Full Volume I main manuscript in casebound edition",
    ...
    y -= 10
    note = (
        "Fulfillment: Your hardcover will be manufactured and shipped on release. "
    ...
    c.drawCentredString(right_x + sig_w / 2, y - 22, "Hardcover Edition")

    c.setFont("Georgia", 7)
    c.setFillColor(MUTED)
    c.drawCentredString(cx, 0.78 * inch, "theveilpress.com  ·  Hardcover Edition")
    c.drawCentredString(
        cx,
        0.64 * inch,
