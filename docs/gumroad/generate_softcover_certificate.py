"""
Generate Softcover Book certificate + receipt PDF for Gumroad delivery.
Run: python generate_softcover_certificate.py
"""
...
OUT = Path(__file__).with_name("Softcover-Book-Certificate-Receipt.pdf")
...
def build():
    ...
    c.setTitle("Softcover Book — Certificate of Purchase & Receipt")
    c.setAuthor("The Veil Press")
    c.setSubject("Softcover edition — certificate and receipt")
    ...
    y -= 28
    draw_centered(c, "SOFTCOVER EDITION", y, "Georgia-Bold", 14, GOLD)
    y -= 16
    draw_centered(c, "Trade Paperback", y, "Georgia-Italic", 12, INK_SOFT)
    ...
    y -= 26
    body = (
        "This document certifies that the holder has purchased the softcover "
        "(trade paperback) edition of The Veil of the Square Mile — Volume I from "
        "The Veil Press."
    )
    y = wrap_centered(
    ...
    y -= 14
    includes = [
        "One softcover (trade paperback) copy of The Veil of the Square Mile",
        "Full Volume I main manuscript",
    ...
    y -= 10
    note = (
        "Fulfillment: Your softcover will be manufactured and shipped on release. "
        "Keep this PDF as your permanent receipt and certificate. "
    ...
    c.drawCentredString(right_x + sig_w / 2, y - 22, "Softcover Edition")

    c.setFont("Georgia", 7)
    c.setFillColor(MUTED)
    c.drawCentredString(cx, 0.78 * inch, "theveilpress.com  ·  Softcover Edition")
    c.drawCentredString(
        cx,
        0.64 * inch,
